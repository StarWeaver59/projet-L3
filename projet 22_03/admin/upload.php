<?php
session_start();
include("../inc/connexion.php");
include("../classes/fichier.php");
// Vérifier si le formulaire a été soumis
if($_SERVER["REQUEST_METHOD"] == "POST"){
    // Vérifie si le fichier a été uploadé sans erreur.
    if(isset($_FILES["monFichier"]) && $_FILES["monFichier"]["error"] == 0){
        $allowed = array("jpg" => "image/jpg", "jpeg" => "image/jpeg", "gif" => "image/gif", "png" => "image/png", "PNG" => "image/PNG", "pdf" => "application/pdf");
        $filename = $_FILES["monFichier"]["name"];
        $filetype = $_FILES["monFichier"]["type"];
        $filesize = $_FILES["monFichier"]["size"];

        // Vérifie l'extension du fichier
        $ext = pathinfo($filename, PATHINFO_EXTENSION);
        if(!array_key_exists($ext, $allowed)) die("Erreur : Veuillez sélectionner un format de fichier valide.");

        // Vérifie la taille du fichier - 5Mo maximum
        $maxsize = 5 * 1024 * 1024;
        if($filesize > $maxsize) die("Error: La taille du fichier est supérieure à la limite autorisée.");

        // Vérifie le type MIME du fichier
        if(in_array($filetype, $allowed)){
            // Vérifie si le fichier existe avant de le télécharger.
            if(file_exists("upload/" . $_FILES["monFichier"]["name"])){
                echo $_FILES["monFichier"]["name"] . " existe déjà.";
            } else{
                move_uploaded_file($_FILES["monFichier"]["tmp_name"], "../fichier/".$_FILES["monFichier"]["name"]);
                echo "Votre fichier a été téléchargé avec succès. <br />";
				echo "Nom du fichier: ".$filename."<br />";
				echo "Type du fichier :".$filetype."<br />";
				echo "Taille du fichier :".$filesize." octet <br />";
            }
        } else{
            echo "Error: Il y a eu un problème de téléchargement de votre fichier. Veuillez réessayer."; 
        }
    } else{
        echo "Error: " . $_FILES["monFichier"]["error"];
    }
	$nom_cours = $_POST['nom_cours'];
	
	$q = "SELECT id_cours FROM cours WHERE nom_cours = '$nom_cours'";
	$result = $bdd->requetev2($q);
	
	$result = mysqli_fetch_row($result);
	
	//var_dump($result);

	$f = new Fichier();
	$f->set_nom($filename);
	$f->set_type($filetype);
	$f->set_size($filesize);
	$f->set_id_cours($result[0]);
	if ($f->enregistrer($bdd))
		print "Ajout fichier ok.";
}
?>